...
import type { QuoteRowPublic, QuotesApiResponse } from "@/types/quote-api";
...
function rowMapFrom(rows: QuoteRowPublic[] | null): Map<string, QuoteRowPublic> {
  const m = new Map<string, QuoteRowPublic>();
  if (!rows) return m;
  for (const r of rows) m.set(r.ticker.toUpperCase(), r);
  return m;
}

function markSource(r: QuoteRowPublic | undefined): string {
  if (!r) return "";
...
export function PositionMonitor({
  ...
  const fetchQuotes = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setClientError("Quotes endpoint returned an error");
        return;
      }
      ...
      setClientError("Could not load quotes");
      setData(null);
  ...
  const statusLine = useMemo(() => {
    ...
    if (!data && clientError) return clientError;
    if (!data) return "Marks not available";
    ...
    const parts: string[] = [`As of ${t}`];
    if (refreshing) parts.push("Refreshing…");
    return parts.join(" · ");
  }, [loading, data, clientError, refreshing]);
  ...
  return (
    <div className="space-y-4">
      {data?.warning ? (
        <div
          className="border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-[12px] leading-snug text-zinc-700"
          role="status"
        >
          {data.warning}
        </div>
      ...
      {clientError ? (
        <div
          className={`border px-3 py-2.5 text-[12px] ${
            data ? "border-zinc-200 bg-zinc-100/60 text-zinc-800" : "border-zinc-300 bg-white text-zinc-800"
          }`}
          role="alert"
        >
          {clientError}
      ...
      <div
        className={`space-y-4 ${initialLoad ? "opacity-50" : ""}`}
      ...
      >
        <div className="hidden overflow-x-auto md:block md:-mx-4 md:px-4 lg:mx-0 lg:px-0">
          ...
          <p className="mt-3 max-w-[38rem] text-[11px] leading-relaxed text-zinc-500">
            Last prices from Finnhub via the server. On failure, values use{" "}
            <span className="font-mono text-zinc-600">offlineQuote</span> in{" "}
            <span className="font-mono text-zinc-600">data/positions.json</span>. Ex.{" "}
            {benchmarkLabel} is since-open excess, not an alpha estimate.
          </p>
        </div>
